...
export function LeaderHome({
  ...
  const handleShare = async () => {
    const text =
      `Voy primero en el PRODE High Rolling con ${myEntry.puntos_totales} puntos 🏆\n` +
      `¿Me venís a buscar?\nhr.prodecaballito.com/ranking`
    if (navigator.share) {
      await navigator.share({ title: 'PRODE High Rolling', text }).catch(() => {})
    } else {
